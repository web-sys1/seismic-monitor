...
try:
 wave1 = NIEP_client.get_waveforms('RO', 'BANR', '--', 'HHZ', start.replace(minute=0, second=1), endtime)
 #wave1.filter('highpass', freq=1, corners=2, zerophase=True)
 wave1.filter("bandpass", freqmin=0.3, freqmax=25.00, corners=3, zerophase=False)
 wave1.detrend(type='demean')
 wave1.detrend(type='linear')
 wave1.plot(type="dayplot", interval=60, size=(1360, 800), dpi=164, number_of_ticks=6, tick_rotation=45,
        right_vertical_labels=False,
        vertical_scaling_range=5e3, one_tick_per_line=True,
        color=['k', 'r', 'b', 'g'], show_y_UTC_label=True,
        outfile='public/waveforms/RO/RO_12H_BANR_HHZ.png')
